...
if (!MONGO_URI) throw new Error('MONGO_URI가 .env에 없습니다.')

// MongoDB 연결
mongoose.connect(MONGO_URI).then(() => console.log('✅ MongoDB 연결 성공')).catch((err) =>
...
// API 라우터 연결 (안내데스크)
// 헬스 체크용
app.get('/api', (req, res) => {res.status(200).send('API 서버 살아있음 (Management)')})
